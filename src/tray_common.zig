...
/// Source resolution of the embedded battery glyph (see tools/gen_battery.zig).
pub const battery_src_px: usize = 32;

/// Anti-aliased white battery glyph, [R,G,B,A] per pixel (RGB white, A =
/// coverage), 32×32. Generated by `zig run tools/gen_battery.zig` and embedded
/// so the tray needs no PNG decoder at runtime. Drawn on the blue on-cable icon
/// in place of the unreliable percentage: the literal 🔋 emoji can't be used
/// (Linux has no font engine, Windows GDI can't render color emoji), so we
/// composite this asset over the icon background instead.
pub const battery_rgba = @embedFile("assets/battery.rgba");

comptime {
    if (battery_rgba.len != battery_src_px * battery_src_px * 4)
        @compileError("battery.rgba size mismatch — re-run tools/gen_battery.zig");
}

/// Coverage alpha (0..255) of the battery glyph at output pixel (x, y) on an
/// n×n icon, area-averaged down from the 32×32 source (n ≤ 32). Pure, so both
/// trays render an identical glyph at their own size (Windows 16px, Linux 22px).
pub fn batteryAlpha(n: usize, x: usize, y: usize) u8 {
...
/// The battery glyph pixel at (x, y) on an n×n icon: the white glyph
/// alpha-composited over `bg`. A transparent source pixel returns `bg` unchanged.
pub fn batteryPixel(n: usize, x: usize, y: usize, bg: Rgb) Rgb {
    return blend(bg, palette.text, batteryAlpha(n, x, y));
}
...
test "batteryPixel: glyph composites over the background at both icon sizes" {
    const bg = palette.blue;
    for ([_]usize{ 16, 22 }) |n| {
        // The body center is fully covered → solid white glyph color.
        try std.testing.expectEqual(@as(u8, 255), batteryAlpha(n, n / 2, n / 2));
        try std.testing.expectEqual(palette.text, batteryPixel(n, n / 2, n / 2, bg));
        // The canvas corners are transparent → background shows through exactly.
        try std.testing.expectEqual(@as(u8, 0), batteryAlpha(n, 0, 0));
        try std.testing.expectEqual(bg, batteryPixel(n, 0, 0, bg));
        try std.testing.expectEqual(bg, batteryPixel(n, n - 1, n - 1, bg));
    }
}
